...
#' @param D_PSW_PVG (numeric) The relative score of P saturation degree for P loss to surface water
...
bbwp_field_indicators <- function(D_NGW_SCR,D_NGW_LEA,D_NGW_NLV,
                                  ...
                                  D_PSW_SCR,D_PSW_GWT,D_PSW_RO,D_PSW_WS,D_PSW_PCC,D_PSW_PVG,D_PSW_PRET,
                                  D_NUE_WRI,D_NUE_PBI,D_NUE_WDRI,D_NUE_NLV,
                                  D_WUE_WWRI,D_WUE_WDRI,D_WUE_WHC){
  
  # check length inputs
  arg.length <- max(length(D_NGW_SCR),length(D_NGW_LEA),length(D_NGW_NLV),
                    length(D_NSW_SCR),length(D_NSW_GWT),length(D_NSW_RO),length(D_NSW_WS),length(D_NSW_NLV),
                    length(D_PSW_SCR),length(D_PSW_GWT),length(D_PSW_RO),length(D_PSW_WS),length(D_PSW_PCC),length(D_PSW_PVG),length(D_PSW_PRET),
  ...
  dt[, D_RISK_PSW := (2 * wf(D_PSW_SCR) * D_PSW_SCR + wf(D_PSW_GWT) * D_PSW_GWT + 2 * wf(D_PSW_RO) * D_PSW_RO + 2 * wf(D_PSW_WS) * D_PSW_WS + wf(D_PSW_PCC) * D_PSW_PCC + wf(D_PSW_PVG) * D_PSW_PVG + wf(D_PSW_PRET) * D_PSW_PRET ) /
        (2 * wf(D_PSW_SCR) + wf(D_PSW_GWT) + 2 * wf(D_PSW_RO) + 2 * wf(D_PSW_WS) + wf(D_PSW_PCC) + wf(D_PSW_PVG) + wf(D_PSW_PRET))]
