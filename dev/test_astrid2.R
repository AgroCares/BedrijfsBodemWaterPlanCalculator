...
######################################################################################################################  
# LOCATION TO STORE CSV TESTS
loc <- 'C:/BBWP-ecoregeling/tests_BenN_221012/'
...
#### test van Sven ####
measures <- rbind(data.table(id = 1, dt.measures[grepl('EB10A|EB10B',eco_id)]),
...
bedrijfsscore_sven <- ecoregeling(B_SOILTYPE_AGR = B_SOILTYPE_AGR,
                              B_GWL_CLASS = B_GWL_CLASS,
                              B_AER_CBS = B_AER_CBS,
                              A_P_SG = A_P_SG,
                              B_SLOPE_DEGREE = B_SLOPE_DEGREE,
...
# get farmscores
farmscores <- as.vector(unlist(bedrijfsscore_sven$farm))[1:10]
scorenames <- as.vector(unlist(names(bedrijfsscore_sven$farm)))[1:10]
out5 <- data.table(thema = scorenames,score = farmscores)
out5a <- as.data.table(bedrijfsscore_sven$fields)
